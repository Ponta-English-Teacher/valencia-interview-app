...
export default function TTSPage() {
  ...
  // ── TTS generation (for student answers only) ──
  ...
  const startRecording = async () => {
    try {
      ...
      recorder.onstop = async () => {
        ...
        await runWhisperTranscription(new Blob(chunks, { type: "audio/webm" }));
      };
  ...
  const runWhisperTranscription = async (blob: Blob) => {
    try {
      ...
      if (!res.ok) throw new Error((await res.text()) || "Failed to transcribe audio.");
  ...
  const runAnswerProcessing = async (mode: "correct" | "natural") => {
    if (!transcript.trim()) { alert("There is no transcript to process yet."); return; }
    ...
    try {
      const res = await fetch("/api/interview-tools", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ mode, answer: transcript, question: currentQuestion.text }),
      });
      if (!res.ok) throw new Error((await res.text()) || "Failed to process answer.");
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
            ...
            {interviewSets.map((s, idx) => (
              <button
                ...
                style={{
                  padding: "4px 10px",
                  borderRadius: 999,
                  border: idx === currentSetIndex ? "2px solid #38bdf8" : "1px solid #4b5563",
                  backgroundColor: idx === currentSetIndex ? "#0f172a" : "#1e293b",
                  color: "white",
                  cursor: "pointer",
                  fontSize: 13,
                }}
            ...
            {valenciaQuestions.map((q, idx) => (
              <button
                ...
                style={{
                  padding: "4px 8px",
                  borderRadius: 6,
                  border: idx === currentQuestionIndex ? "2px solid #38bdf8" : "1px solid #4b5563",
                  backgroundColor: idx === currentQuestionIndex ? "#0f172a" : "#1e293b",
                  color: "white",
                  cursor: "pointer",
                  fontSize: 12,
          ...
          <div style={{ marginBottom: 4 }}>
            ...
            <span style={{ fontSize: 16 }}>{currentQuestion.text}</span>
          </div>
          <div style={{ fontSize: 13, color: "#9ca3af", marginBottom: 2 }}>
            <strong>Hint:</strong> {currentQuestion.hint}
          </div>
          <div style={{ fontSize: 12, color: "#6b7280", marginBottom: 10 }}>
            Recommended length: {currentQuestion.targetSeconds.min}–{currentQuestion.targetSeconds.max} seconds
          </div>

          {/* Pre-generated question audio */}
          <div
            style={{
              display: "flex",
              gap: 16,
          ...
          >
            ...
            <MiniPlayer
              src={`/audio/q${currentQuestion.id}.mp3`}
              label="Question"
            />
            <MiniPlayer
              src={`/audio/q${currentQuestion.id}-model.mp3`}
              label="Model answer"
            />
          </div>
          ...
          {showModelAnswer && (
            ...
            >
              <div style={{ fontWeight: 600, marginBottom: 4 }}>Model answer (example):</div>
          ...
          <div style={{ display: "flex", gap: 8 }}>
            ...
            <button
              ...
              style={{
                padding: "6px 12px", borderRadius: 6, border: "none",
                background: currentSetPos === setLength - 1 ? "#4b5563" : "#38bdf8",
                color: currentSetPos === setLength - 1 ? "white" : "black",
                cursor: currentSetPos === setLength - 1 ? "default" : "pointer",
                fontSize: 13,
              }}
            >
              Next ▶
        ...
        <div style={{ display: "flex", gap: 24, alignItems: "flex-start" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            {(isRecording || isTranscribing) && (
              <div style={{ fontSize: 12, color: "#a5b4fc", marginBottom: 4 }}>
                {isRecording ? "🎙 Recording… Speak your answer." : "⏳ Transcribing with Whisper…"}
              </div>
            )}
            {sttError && (
            ...
            <textarea
              ...
              style={{
                width: "100%", height: "140px", padding: 12, borderRadius: 8,
                background: "#0f172a", color: "white", fontSize: 14, lineHeight: 1.5,
                border: "1px solid #334155", marginBottom: 8, resize: "vertical",
              }}
            />
            ...
            <button
              type="button"
              onClick={() => generateAudio(transcript, "transcript")}
              disabled={!transcript.trim() || ttsLoading}
              style={btn("#38bdf8", "black", !transcript.trim() || ttsLoading)}
            >
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
              <button
                ...
                style={btn("#0ea5e9", "black", !transcript.trim() || processingMode !== null)}
              >
                {processingMode === "correct" ? "Correcting…" : "✏️ Correction"}
              </button>
              ...
              <button
                ...
                disabled={!transcript.trim() || processingMode !== null}
                style={btn("#a855f7", "white", !transcript.trim() || processingMode !== null)}
              >
            ...
            {processedAnswer && (
              ...
              >
                <div style={{ fontWeight: 600, marginBottom: 6, color: "#e5e7eb" }}>
                  {processingMode === null ? "Processed answer:" : ""}
                </div>
                <div style={{ marginBottom: 8 }}>{processedAnswer}</div>
                <button
                  type="button"
                  onClick={() => generateAudio(processedAnswer, "corrected")}
                  disabled={ttsLoading}
                  style={btn("#22c55e", "black", ttsLoading)}
                >
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
              {voices.map((v) => (
                <div
                  ...
                  style={{
                    border: voice === v.voice ? "3px solid #38bdf8" : "2px solid #475569",
                    borderRadius: 12, padding: "12px 8px", cursor: "pointer",
                    textAlign: "center", background: "#1e293b", transition: "0.15s",
                  }}
                >
                  <img
                    src={v.img}
                    style={{
                      ...
                      borderRadius: "50%", margin: "0 auto 6px auto", display: "block",
                    }}
                  />
            ...
            {/* Player (only shown after audio is generated) */}
            {audioUrl && (
              <div
                style={{
              ...
              >
                ...
                <div style={{ fontSize: 12, color: "#9ca3af", marginBottom: 8 }}>
                  {ttsSource === "transcript" ? "▶ Your answer audio" : "▶ Corrected answer audio"}
                </div>
                <button
                  onClick={playAudio}
                  style={{
                    ...
                    cursor: "pointer", fontSize: 14, color: "black", fontWeight: 600,
                  }}
                >
                  ▶ Play
                </button>
      ...
      <footer
        style={{
          marginTop: 40, textAlign: "center", fontSize: 14,
          color: "#94a3b8", borderTop: "1px solid #334155",
          paddingTop: 12, opacity: 0.9,
        }}
      >
